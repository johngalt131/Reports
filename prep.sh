...
sed -n '1,/BEGINCASE/ p' < $FUNCTION_CASE_FILE | sed '$ d' > tmp
sed -n '/BEGINCASE/,/ENDCASE/ p' < $GETTER_SETTER_METHODS_FILE >> tmp
sed -n '/ENDCASE/,$ p' < $FUNCTION_CASE_FILE  | sed '1 d' >> tmp
mv tmp $FUNCTION_CASE_FILE
indent $FILE_NAME
sed "s/\t/$TAB/" <$FILE_NAME > tmp && mv tmp $FILE_NAME


sed -n '1,/BEGINPROTOTYPES/ p' < $FUNCTION_PROTOTYPE_FILE | sed '$ d' > tmp
sed -n '/BEGINPROTOTYPES/,/ENDPROTOTYPES/ p' < $GETTER_SETTER_METHODS_FILE >> tmp
sed -n '/ENDPROTOTYPES/,$ p' < $FUNCTION_PROTOTYPE_FILE | sed '1 d' >> tmp
...
sed -n '1,/BEGINIMPLEMENTATION/ p' < $FUNCTION_IMPLEMENTATIONS_FILE | sed '$ d' > tmp
sed -n '/BEGINIMPLEMENTATION/,/ENDIMPLEMENTATION/ p' < $GETTER_SETTER_METHODS_FILE >> tmp
sed -n '/ENDIMPLEMENTATION/,$ p' < $FUNCTION_IMPLEMENTATIONS_FILE | sed '1 d' >> tmp
mv tmp $FUNCTION_IMPLEMENTATIONS_FILE

sed -n '1,/BEGINCONSTRUCTOR/ p' < $FUNCTION_IMPLEMENTATIONS_FILE | sed '$ d' > tmp
sed -n '/BEGINCONSTRUCTOR/,/ENDCONSTRUCTOR/ p' < $GETTER_SETTER_METHODS_FILE >> tmp
sed -n '/ENDCONSTRUCTOR/,$ p' < $FUNCTION_IMPLEMENTATIONS_FILE | sed '1 d' >> tmp
mv tmp $FUNCTION_IMPLEMENTATIONS_FILE
